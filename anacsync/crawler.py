        ...
        with HTTPClient(self.config) as http_client:
            ...
            with Progress(
                ...
                while empty_pages < self.config.crawler.empty_page_stop_after:
                    console.print(f"[blue]DEBUG: Crawling page {page_num}...[/blue]")
                    datasets, has_content = self.crawl_page(page_num, http_client)
                    console.print(f"[blue]DEBUG: Page {page_num} result: {len(datasets)} datasets, has_content: {has_content}[/blue]")
